...
def generate_perlin_noise(size, scale=100.0):
    ...
    g01 = gradients[:-1, 1:]
    g11 = gradients[1:, 1:]
    n00 = np.sum(np.dstack((grid[..., np.newaxis], -grid[..., np.newaxis])) * g00[:, :, np.newaxis, :], axis=4)
    n10 = np.sum(np.dstack((grid - 1, -grid[..., np.newaxis])) * g10[None, None, :, :], axis=4)
    n01 = np.sum(np.dstack((grid[..., np.newaxis], 1 - grid[..., np.newaxis])) * g01[None, None, :, :], axis=4)
    n11 = np.sum(np.dstack((grid - 1, 1 - grid[..., np.newaxis])) * g11[None, None, :, :], axis=4)
    t = f(grid)
    n0 = n00 + t[..., 0] * (n10 - n00)
    n1 = n01 + t[..., 0] * (n11 - n01)
